...
1. Create a MySQL database in Hostinger and note the database host, username, password, database name, and port. You can inspect and manage the database tables in phpMyAdmin.
...
The app uses Sequelize models with foreign keys and indexes. `npm run db:sync` creates the MySQL schema, and `npm run db:seed` inserts realistic sample data. A readable SQL schema is also provided at `backend/config/schema.sql` for review or manual phpMyAdmin imports.
